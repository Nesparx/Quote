...
# --- 1. SAFETY INITIALIZATION (FIXES THE ATTRIBUTE ERROR) ---
...
# Function to force a state update
def refresh():
    pass 
...
def get_current_totals():
    ...
    tiered_sm_count = sum(1 for l in lines if l.get('plan') in SMARTPHONE_TIERS)
    ...
    for idx, l in enumerate(lines):
        ...
        elif dtype == "Internet": base = INTERNET.get(plan, {"price": 0})['price']
        ...
        # Protection
        prot = VBIS_PROTECTION.get(l.get('vbis', 'None'), 0.0) if dtype == "Internet" else SINGLE_PROTECTION.get(l.get('protection', 'None'), 0.0)
...
if st.session_state.step == 1:
    ...
    for i in range(st.session_state.num_lines):
        with st.expander(f"Line {i+1} Plan", expanded=True):
            st.session_state.lines[i]['type'] = st.selectbox("Category", ["Smartphone", "Internet", "Tablet", "Watch"], key=f"t_{i}", on_change=refresh)
            ...
            else: opts = list(WATCHES.keys())
            st.session_state.lines[i]['plan'] = st.selectbox("Select Plan", opts, key=f"p_{i}", on_change=refresh)
    if st.button("Next"): st.session_state.step = 3; st.rerun()

elif st.session_state.step == 3:
    st.header("Step 3: Account Options")
    st.toggle("Autopay", key="autopay", on_change=refresh)
    st.toggle("Military", key="military", on_change=refresh)
    ...
    if has_sm and has_int:
        st.toggle("Joint Offer", key="joint_offer", on_change=refresh)
    
    eligible_count = sum(1 for l in st.session_state.lines if l['type'] in ["Smartphone", "Tablet", "Watch"])
    multi_opts = ["None"]
    if eligible_count >= 3:
        for k in MULTI_PROTECTION_DATA.keys(): multi_opts.append(k)
    st.selectbox("Multi-Device Protection", multi_opts, key="tmp_multi", on_change=refresh)
    st.toggle("Whole Office Protect", key="whole_office", on_change=refresh)
    ...
    for i in range(st.session_state.num_lines):
        ...
        with st.expander(f"Line {i+1} ({l['plan']}) - {l_info[i]['tier']} Tier", expanded=(i==0)):
            if l['type'] == "Internet":
                l['vbis'] = st.selectbox("Security", list(VBIS_PROTECTION.keys()), key=f"vbis_{i}", on_change=refresh)
            else:
                l['protection'] = st.selectbox("Protection", ["None"] + list(SINGLE_PROTECTION.keys()), key=f"pr_{i}", on_change=refresh)
            
            if l['plan'] == "My Biz":
                l['features'] = [f for f in ADDONS if st.checkbox(f"{f} (${ADDONS[f]['price']})", key=f"a_{i}_{f}", on_change=refresh)]
                l['intro_disc'] = st.checkbox("15% Intro Discount", key=f"id_{i}", on_change=refresh)
            
            l['dev_pay'] = st.number_input("Monthly Device Payment", min_value=0.0, key=f"dp_{i}", on_change=refresh)
